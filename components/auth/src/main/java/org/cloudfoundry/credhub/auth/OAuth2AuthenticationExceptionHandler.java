...
public class OAuth2AuthenticationExceptionHandler extends OAuth2AuthenticationEntryPoint {
  ...
  private boolean tokenIsExpired(final Map<String, Object> tokenInformation) {
    final Long exp = tokenInformation != null ? (Long) tokenInformation.get(AccessTokenConverter.EXP) : null;
  ...
  private Map<String, Object> extractTokenInformation(final String token) {
    try {
      ...
      if (map.containsKey(AccessTokenConverter.EXP) && map.get(AccessTokenConverter.EXP) instanceof Integer) {
        final Integer intValue = (Integer) map.get(AccessTokenConverter.EXP);
        map.put(AccessTokenConverter.EXP, Long.valueOf(intValue));
      }
