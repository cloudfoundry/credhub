    ...
    override fun migrate(jdbcTemplate: JdbcTemplate) {
        ...
        for (credentialUuid in credentialsWithEncryptedValues) {
            ...
            jdbcTemplate.update(
                "insert into encrypted_value (" +
                    ...
                    "where uuid = ?",
                arrayOf(encryptedValueUuid, credentialUuid), intArrayOf(Types.VARBINARY, Types.VARBINARY)
            )
            jdbcTemplate.update(
        ...
        for (passwordCredentialUuid in passwordsWithEncryptedValues) {
            ...
            jdbcTemplate.update(
                ...
                arrayOf(encryptedValueUuid, passwordCredentialUuid), intArrayOf(Types.VARBINARY, Types.VARBINARY)
            )
            jdbcTemplate.update(
        ...
        for (userCredentialUuid in usersWithEncryptedValues) {
            ...
            jdbcTemplate.update(
                "insert into encrypted_value (" +
                    ...
                    "credential_version.uuid = ?;",
                arrayOf(encryptedValueUuid, userCredentialUuid), intArrayOf(Types.VARBINARY, Types.VARBINARY)
            )
            jdbcTemplate.update(
