...
import io.pivotal.security.service.*;
...
public class EncryptionKeyRotatorTest {
...

  {
    ...
    describe("rotation", () -> {
      ...
      it("can rotate password secrets", () -> {
        ...
        final NamedPasswordSecret firstEncryption = (NamedPasswordSecret) secretDataService.findAllByName("/cred1").get(0);

        setActiveKey(1);

        encryptionKeyRotator.rotate();

        final NamedPasswordSecret secondEncryption = (NamedPasswordSecret) secretDataService.findAllByName("/cred1").get(0);
        assertThat(firstEncryption.getEncryptedValue(), not(equalTo(secondEncryption.getEncryptedValue())));
        assertThat(firstEncryption.getEncryptedGenerationParameters(), not(equalTo(secondEncryption.getEncryptedGenerationParameters())));
      ...
      it("can rotate certificate secrets", () -> {
        ...
        final NamedCertificateSecret firstEncryption = (NamedCertificateSecret) secretDataService.findAllByName("/cred1").get(0);

        setActiveKey(1);

        encryptionKeyRotator.rotate();

        final NamedCertificateSecret secondEncryption = (NamedCertificateSecret) secretDataService.findAllByName("/cred1").get(0);
        assertThat(firstEncryption.getEncryptedValue(), not(equalTo(secondEncryption.getEncryptedValue())));
  ...
  private void setActiveKey(int index) {
    ...
    for(EncryptionKeyMetadata encryptionKeyMetadata : encryptionKeysConfiguration.getKeys()) {
      keys.add(new EncryptionKeyMetadata(encryptionKeyMetadata.getDevKey(), encryptionKeyMetadata.getEncryptionKeyName(), false, encryptionKeyMetadata.getEncryptionPassword()));
    }
