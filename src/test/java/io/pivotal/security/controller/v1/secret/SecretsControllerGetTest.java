...

  {
    ...
    describe("getting a secret", () -> {
      ...
      beforeEach(() -> {
        ...
        NamedValueSecret valueSecret = new NamedValueSecret(secretName).setEncryptor(encryptor)
            .setUuid(uuid).setVersionCreatedAt(frozenTime);
        valueSecret.setEncryptedValue("fake-encrypted-value1".getBytes());
        valueSecret.setEncryptedValue("fake-encrypted-value2".getBytes());
        NamedValueSecret valueSecret2 = new NamedValueSecret(secretName).setEncryptor(encryptor)
            .setUuid(uuid).setVersionCreatedAt(frozenTime);
      ...
      describe(
          "getting a secret by name case-insensitively (with name query param, and"
              + " no leading slash)",
          makeGetByNameBlock(secretValue, "/api/v1/data?name=" + secretName.toUpperCase(),
    ...
    describe("when key not present", () -> {
      beforeEach(() -> {
        ...
        doThrow(new KeyNotFoundException()).when(encryptor)
            .decrypt(any(UUID.class), any(byte[].class), any(byte[].class));
        ...
        mockMvc.perform(get)
            ...
            .andExpect(jsonPath("$.error")
                .value("The credential could not be accessed with the provided"
                    + " encryption keys. You must update your deployment configuration to "
                    + "continue."));
      });
    });
  }
    ...
    return () -> {
      ...
      it("returns NOT_FOUND when the secret does not exist", () -> {
        ...
        mockMvc.perform(get)
            ...
            .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
            .andExpect(jsonPath("$.error")
                .value("Credential not found. Please validate your input and retry your request."));
      });
    };
  }
