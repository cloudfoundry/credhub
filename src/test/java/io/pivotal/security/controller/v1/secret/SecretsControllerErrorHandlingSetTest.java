        ...
        describe("error handling", () -> {
          it("returns 400 when the handler raises an exception", () -> {
            ...
            final MockHttpServletRequestBuilder put = put("/api/v1/data")
                ...
                .content("{"
                    + "  \"type\":\"password\","
                    + "  \"name\":\""
                    + secretName + "\",  \"value\":\"some password\""
                    + "}");
            ...
            mockMvc.perform(put)
                ...
                .andExpect(jsonPath("$.error").value(
                    "The credential type cannot be modified."
                        + " Please delete the credential if you wish to create"
          ...
          it("returns 400 when name is empty", () -> {
            ...
            mockMvc.perform(put)
                ...
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.error").value(
                    "A credential name must be provided. Please validate your input"
          ...
          it("returns 400 when name contains double slash (//)", () -> {
            ...
            mockMvc.perform(put)
                ...
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.error").value(
                    "A credential name cannot end with a '/' character or contain '//'."
                        + " Credential names should be in the form of /[path]/[name] or"
                        + " [path]/[name]. Please update and retry your request."));
          });
          ...
          it("returns an error message when an unknown top-level key is present", () -> {
            final MockHttpServletRequestBuilder put = put("/api/v1/data")
                ...
                .content("{"
                    + "  \"type\":\"value\","
                    + "  \"name\":\""
                    + secretName + "\",  \"response_error\":\"invalid key\","
            ...
            mockMvc.perform(put)
                ...
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.error").value(
                    "The request includes an unrecognized parameter 'response_error'."
          ...
          it("returns an error message when the input JSON is malformed", () -> {
            ...
            mockMvc.perform(put)
                ...
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.error").value(
                    "The request could not be fulfilled because the request path "
                        + "or body did not meet expectation. Please check the documentation for "
            ...
            it("returns a nice error message", () -> {
              ...
              this.mockMvc.perform(post).andExpect(status().isBadRequest())
                  ...
                  .andExpect(jsonPath("$.error", equalTo(
                      "The request could not be fulfilled because the request path or"
      ...
      it("should return 400 when trying to update a secret with a mismatching type", () -> {
        final MockHttpServletRequestBuilder put = put("/api/v1/data")
            ...
            .content("{"
                + "  \"type\":\"password\","
                + "  \"name\":\""
                + secretName.toUpperCase() + "\",  \"value\":\"my-password\","
                + "  \"overwrite\":true"
                + "}");
