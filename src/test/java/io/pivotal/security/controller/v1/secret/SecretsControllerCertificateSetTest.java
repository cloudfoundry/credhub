    ...
    describe("setting certificates", () -> {
      describe("when required values are passed", () -> {
        ...
        it("returns the secret as json", () -> {
          NamedCertificateSecret expected = (NamedCertificateSecret) secretDataService
              .findMostRecent(secretName);

          assertThat(expected.getPrivateKey(),
              equalTo("-----BEGIN RSA PRIVATE KEY-----fake-----END RSA PRIVATE KEY-----"));

          response.andExpect(status().isOk())
              .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
              .andExpect(jsonPath("$.type").value("certificate"))
              .andExpect(jsonPath("$.value.private_key")
                  .value("-----BEGIN RSA PRIVATE KEY-----fake-----END RSA PRIVATE KEY-----"))
              .andExpect(jsonPath("$.id").value(expected.getUuid().toString()));
        });
      });

      describe("when all values are empty", () -> {
        it("should return an error with the message"
                + " 'At least one certificate attribute must be set. "
            ...
            ()
              ...
              final MockHttpServletRequestBuilder put = put("/api/v1/data")
                  ...
                  .content("{"
                      ...
                      + "  }"
                      + "}");
              final String errorMessage = "At least one certificate attribute must be set."
                  + " Please validate your input and retry your request.";
              mockMvc.perform(put)
