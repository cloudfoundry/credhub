...
public class DecryptableDataDetector {
  ...
  DecryptableDataDetector(EncryptionKeyCanaryMapper encryptionKeyCanaryMapper, SecretDataService secretDataService) {
  ...
  public void check() {
    ...
    if (countTotalSecrets > 0 && countSecretsEncryptedWithKeyWeHave == 0) {
      throw new RuntimeException("The encryption keys provided cannot decrypt any of the " + countTotalSecrets + " value(s) in the database. Please make sure you've provided the necessary encryption keys.");
    }
  }
}